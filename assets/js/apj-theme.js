...
(function(){
  ...
  function notify(message, type){
    if (window.APJPermissionNotice && type === 'permission-soon') {
      return window.APJPermissionNotice.show({ type: 'soon', menuName: message || 'Menu ini' });
    }
    ...
    // Fallback terakhir tanpa browser alert; cukup console agar tidak dobel dengan modal permission.
...
(function(){
  ...
  function getPageLabel(file){
    var map = {
      ...
      'stok-opname.html':'Stok Opname', 'lihat-stok.html':'Lihat Stok', 'riwayat-inventory.html':'Riwayat Inventory', 'absensi.html':'Absensi Saya', 'dashboard-absensi.html':'Dashboard Absensi', 'admin.html':'Admin Sistem'
    };
  ...
  function ensureLockBadge(link){
    ...
    if (oldSoon) return;
    // V181: jangan bikin tulisan Lock dobel dengan apj-permission-sync.
    link.querySelectorAll('.nav-lock-badge,.apj-lock-badge').forEach(function(el){ el.remove(); });
  ...
  function syncSidebarLocks(){
    ...
    document.querySelectorAll('.nav-locked').forEach(function(link){
      ...
      link.addEventListener('click', function(event){
        ...
        if (window.APJPermissionNotice) window.APJPermissionNotice.show({ type: 'locked', menuName: name });
        else if (window.APJToast && typeof window.APJToast.warning === 'function') window.APJToast.warning(name + ' terkunci untuk akun ini.');
  ...
  function init(){
    // V182: permission final ditangani oleh apj-permission-sync.js.
    // Legacy gate di theme tidak dijalankan supaya tidak mengunci menu dengan key lama.
    if (window.APJPermissionSync) {
      setTimeout(function(){
        if (window.APJPermissionSync && typeof window.APJPermissionSync.syncStaticSidebar === 'function') {
          window.APJPermissionSync.syncStaticSidebar();
        }
      }, 120);
      return;
    }
