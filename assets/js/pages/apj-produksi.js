...
(function () {
  ...
  async function initPage() {
    ...
    setTimeout(() => {
      if (!apjHasSeenGuide('produksi.html')) openProduksiHelpModal(true);
    }, 450);
  ...
  function hasAccess() {
    if (window.APJPermissionSync && APJPermissionSync.hasAny) return APJPermissionSync.hasAny('produksi');
    const level = localStorage.getItem(STORAGE_KEYS.level) || '';
    if (/owner|super/i.test(level)) return true;
    try {
      const perms = JSON.parse(localStorage.getItem(STORAGE_KEYS.permissions) || '{}');
      return perms.produksi === 'Y' || perms.produksi === true || perms.inventory === 'Y';
    } catch (err) {
      return false;
    }
  }
  ...
  function openProduksiHelpModal(autoOpen = false) {
    ...
    if (autoOpen) apjMarkGuideSeen('produksi.html');
  ...
  function closeProduksiHelpModal() {
    ...
    const content = modal ? modal.querySelector('.modal-content') : null;
    if (!modal || !overlay || !content) return;
    apjMarkGuideSeen('produksi.html');
