...
(function () {
  ...
  function hasAccess() {
    if (window.APJPermissionSync && APJPermissionSync.hasAny) return APJPermissionSync.hasAny('preparasi');
    const level = localStorage.getItem(STORAGE_KEYS.level) || '';
    if (/owner|super/i.test(level)) return true;
    try {
      const perms = JSON.parse(localStorage.getItem(STORAGE_KEYS.permissions) || '{}');
      return perms.preparasi === 'Y' || perms.preparasi === true || perms.inventory === 'Y';
    } catch (err) {
      return false;
    }
  }
