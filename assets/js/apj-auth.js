...
(function () {
  ...
  function hasPermission(permissionKey) {
    ...
    if (window.APJPermissionSync && window.APJPermissionSync.hasAny) return window.APJPermissionSync.hasAny(permissionKey);
    const session = getSession();
    const level = String(session.level || '').toUpperCase();
    if (level === 'OWNER' || level === 'SUPERADMIN') return true;
    return String(permissionKey || '').split(',').map(function(k){ return k.trim(); }).filter(Boolean).some(function(k){
      return normalizeBool(session.permissions && (session.permissions[k] || session.permissions[String(k).toUpperCase().replace(/[^A-Z0-9]+/g, '')]));
    });
  }
